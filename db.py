...
def seed_data():
    ...
    # sample doctors
    c.executemany(
        "INSERT INTO providers(domain,name,specialization,slots) VALUES (?,?,?,?)",
        [
            (
                "doctor",
                "Dr. Priya Sharma",
                "Cardiologist",
                "2025-11-10T10:00,2025-11-10T11:00,2025-11-11T15:00",
            ),
            (
                "doctor",
                "Dr. Raj Mehta",
            ...
            (
                ...
                "Plumber Rahul",
                "Plumber",
                "2025-11-10T10:00,2025-11-10T12:00,2025-11-11T09:00",
            ),
...
def get_providers(domain, specialization=None):
    ...
    if specialization:
        c.execute(
            "SELECT id,name,specialization,slots FROM providers WHERE domain=? AND specialization LIKE ?",
            (domain, f"%{specialization}%"),
        )
    else:
        c.execute(
            "SELECT id,name,specialization,slots FROM providers WHERE domain=?",
            (domain,),
        )
    res = c.fetchall()
    conn.close()
...
def book_provider(domain, provider_id, user_name, time_iso):
    ...
    c.execute(
        "SELECT slots FROM providers WHERE id=? AND domain=?", (provider_id, domain)
    )
    row = c.fetchone()
    if not row:
        conn.close()
    ...
    c.execute(
        "INSERT INTO bookings(domain,provider_id,user_name,time,created_at) VALUES (?,?,?,?,?)",
